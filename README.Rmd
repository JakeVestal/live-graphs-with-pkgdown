...
This package (and its GitHub Pages branch & site) were written for my own
...
1) Open RStudio. You probably need to 'Run as Admin' -- whatever that means for your particular OS.
2) Update all of your packages. Probably should go ahead and make sure you're running the most recent versions of R and RStudio as well.
...
```
4) Create a new 'gh-pages' branch on your repo:
```
...
6) run `usethis::use_pkgdown_github_pages()`
7) Knit your README.Rmd
8) Check your DESCRIPTION file and make sure the `SystemRequirements`,
`VignetteBuilder`, `Imports`, `Depends`, and `Suggests` fields include the objects listed in the DESCRIPTION file in this repo (you may add more but deleting something may break your site).
9) run `pkgdown::build_site()` to preview the site
10) Push to Git.
